...
class TestConfiguration(absltest.TestCase):
    ...
    def test_initialize_from_keyframe(self):
        ...
        q_ref = self.model.key("home").qpos
        np.testing.assert_array_equal(configuration.q, q_ref)

    def test_initialize_from_qpos(self):
        """Test initialization from qpos."""
        qpos = np.zeros(self.model.nq)
        configuration = mink.Configuration(self.model, qpos)
    ...
    def test_inplace_integration(self):
        ...
        configuration = mink.Configuration(self.model)
        configuration.update(q=self.model.key("home").qpos)
        expected_qpos = configuration.q + 1e-3 * qvel
